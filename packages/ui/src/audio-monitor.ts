...
export class AudioMonitor {
	...
	public readonly waveform = [new Float32Array(AUDIO_MONITOR_SETTINGS.fftSize), new Float32Array(AUDIO_MONITOR_SETTINGS.fftSize)];
	...
	public read() {
		...
		for (let channel = 0; channel < 2; channel++) {
			if (this.context.state === 'running') {
				this.analysers[channel].getFloatTimeDomainData(this.waveform[channel]);
