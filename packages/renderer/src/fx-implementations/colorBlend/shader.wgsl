...
fn blendComponents(a: vec3f, b: vec3f) -> vec3f {
	switch uniforms.blendMode {
		...
		case 7u: { return select(1.0 - 2.0 * (1.0 - a) * (1.0 - b), 2.0 * a * b, a <= vec3f(0.5)); }
...
fn fs(@location(0) position: vec2f) -> @location(0) vec4f {
	...
	let blended = clamp(blendComponents(straightA, straightB), vec3f(0.0), vec3f(1.0));
