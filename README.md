...
- [**`renderer`**](./packages/renderer) ... レンダラー実装が含まれます。レンダラーはパラメータを受け取り、WebGPUを用いてレンダリングを行います。なお、レンダラーはWebWorkers上で動作し、オフスクリーンでレンダリングを行うことができます。そのため、DOMに依存しない実装になっています。`ui`を参照することはありません。
