...
- **`shared`** ... `renderer`からも`ui`からも使用される共通の処理や定義などが含まれます。`renderer`、`ui`を参照することはありません。
- **`renderer`** ... レンダラー実装が含まれます。レンダラーはパラメータを受け取り、WebGPUを用いてレンダリングを行います。なお、レンダラーはWebWorkers上で動作し、オフスクリーンでレンダリングを行うことができます。そのため、DOMに依存しない実装になっています。`ui`を参照することはありません。
- **`ui`** ... UI実装が含まれます。状態の管理・レンダラーの呼び出し、表示などを行います。
