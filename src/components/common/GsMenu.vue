...
>
	...
	>
		<template v-for="item in (items2 ?? [])">
			...
			>
				<i v-if="item.icon" class="ti-fw" :class="[$style.icon, item.icon]"></i>
				<GsSwitchButton v-else :class="$style.switchButton" :checked="item.ref" :disabled="item.disabled" @toggle="switchItem(item)"/>
				<div :class="$style.item_content">
					<div :class="[$style.item_content_text, { [$style.switchText]: !item.icon }]">
						<div :class="$style.item_content_text_title">{{ item.text }}</div>
						<div v-if="item.caption" :class="$style.item_content_text_caption">{{ item.caption }}</div>
					</div>
					<GsSwitchButton v-if="item.icon" :class="[$style.switchButton, $style.caret]" :checked="item.ref" :disabled="item.disabled" @toggle="switchItem(item)"/>
				</div>
			</button>
...
async function showRadioOptions(item: MenuRadio, ev: MouseEvent | PointerEvent | KeyboardEvent) {
	...
	if (props.asDrawer) {
		ui.popupMenu(children, ev.currentTarget ?? ev.target).finally(() => {
			close(false);
		});
		emit('hide');
	} else {
...
.item {
	...
	&:not(:disabled) {
		...
		&:focus-visible.active {
			color: var(--menuHoverFg, var(--THEME-accent));
			position: relative;
			z-index: 10; // guardより上にする

			&::before {
				background-color: var(--menuHoverBg, var(--THEME-accentedBg));
			}
		}

		&:not(:focus-visible):active,
		&:not(:focus-visible).active {
			color: var(--menuActiveFg, var(--THEME-fgOnAccent));

			&::before {
				background-color: var(--menuActiveBg, var(--THEME-accent));
			}
		}
	}
	...
	&.danger {
		--menuFg: var(--THEME-error);
		--menuHoverFg: #fff;
		--menuHoverBg: var(--THEME-error);
		--menuActiveFg: #fff;
		--menuActiveBg: hsl(from var(--THEME-error) h s calc(l - 10));
	}

	&.radio {
		--menuActiveFg: var(--THEME-accent);
		--menuActiveBg: var(--THEME-accentedBg);
	}
...
.radioIcon {
	...
	border-radius: 50%;
	border: solid 2px var(--THEME-divider);
