...
import shaderSrc from './mask.glsl';
...
export default defineGpuFx({
	...
	main: ({ renderer, gl, resultFrameBuffer, width, height, params, inputNodeTexs }) => {
		...
		gl.bindTexture(gl.TEXTURE_2D, inputNodeTexs.mask);
		const u_texture_mask = gl.getUniformLocation(shader, 'u_texture_mask');
		gl.uniform1i(u_texture_mask, 2);

		const u_mix = gl.getUniformLocation(shader, 'u_mix');
		gl.uniform1f(u_mix, params.mix);
